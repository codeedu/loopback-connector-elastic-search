...
function create(model, data, done) {
  ...
  */
  ...
  const joinPropertyKey = Object.keys(self.settings.mappingProperties).find(prop => prop === "join");
  if(joinPropertyKey in data && typeof data[joinPropertyKey]=== "object" && "parent" in data[joinPropertyKey]){
    document.routing = data[joinPropertyKey]['parent'];  
  }
  document.body = {};
