...
function buildDeepNestedQueries(
  ...
  _.forEach(where, (value, key) => {
    ...
    if (key === 'and' && Array.isArray(value)) {
      ...
      let spec = false;
      let options = null;
      if (cond && cond.constructor.name === 'Object' && key!=='fuzzy' && key!=='has_parent' && key!=='has_child' && key!=='query_string') { // need to understand
      ...
      if (spec) {
        ...
        if (typeof value === 'string') {
          ...
          if (key === "fuzzy") {
            nestedQuery[typeof value.fields === "undefined" ? "match": "multi_match"] = typeof value.fields === "undefined"
                ? _.mapValues(value, function(value){ return Object.assign({fuzziness: 'AUTO'}, value) })
                : Object.assign({fuzziness: 'AUTO'}, value);
          } else if (key === 'query_string') {
            nestedQuery.query_string = value;
          } else if (key=="has_parent"){
            nestedQuery.has_parent = value;  
          } else if (key=="has_child"){
            nestedQuery.has_child = value;  
          } else { 
            nestedQuery.match = {};
            nestedQuery.match[key] = value;
          }
